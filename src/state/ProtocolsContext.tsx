...
import { allProtocols } from '@/state/fixtures/protocolsMock'
import type { Protocol, ProtocolPayment, ProtocolService } from '@/shared/types'
import { loadFromStorage, saveToStorage } from '@/shared/lib/storage'
import { formatBarcode, loadBarcodeSettings } from '@/shared/lib/barcodeSettings'
import { normalizePaymentType } from '@/shared/lib/payments'
...
let serviceIdSeq = allProtocols.reduce(
  (max, p) => Math.max(max, p.id, ...p.services.map((service) => service.id), ...p.payments.map((payment) => payment.id)),
  0
)
...
function deriveProtocolStatus(services: ProtocolService[], fallback: string) {
  if (services.length === 0) return fallback
  if (services.every((service) => service.status === 'Onaylandı')) return 'Tamamlandı'
  if (services.some((service) => service.status === 'Sonuç Girildi' || service.status === 'Onaylandı')) {
    return 'Sonuç Bekleniyor'
  }
  return 'Bekliyor'
}

function normalizeProtocols(items: Protocol[]): Protocol[] {
  return items.map((protocol) => {
    const services = Array.isArray(protocol.services) ? protocol.services : []
    return {
      ...protocol,
      services,
      status: deriveProtocolStatus(services, protocol.status),
      payments: Array.isArray(protocol.payments)
        ? protocol.payments.map((payment) => ({ ...payment, paymentType: normalizePaymentType(payment.paymentType) }))
        : [],
    }
  })
}
...
export function ProtocolsProvider({ children }: { children: React.ReactNode }) {
  const [protocols, setProtocols] = useState<Protocol[]>(() => normalizeProtocols(loadFromStorage(STORAGE_KEY, allProtocols)))
  ...
  const addProtocol = (protocol: Omit<Protocol, 'id' | 'protocolNo'>) => {
    serviceIdSeq = Math.max(serviceIdSeq + 1, Date.now())
  ...
  const addServiceToProtocol = (
    ...
    setProtocols((prev) =>
      prev.map((p) =>
        p.id === protocolId
          ? { ...p, services: [newService, ...p.services], status: deriveProtocolStatus([newService, ...p.services], p.status) }
          : p
      )
    )
  }
  ...
  const updateServiceInProtocol = (
    ...
    setProtocols((prev) =>
      ...
      .map((p) => p.id === protocolId ? { ...p, status: deriveProtocolStatus(p.services, p.status) } : p)
    )
  }
      ...
      prev.map((p) =>
        p.id === protocolId
          ? {
              ...
              status: deriveProtocolStatus(p.services.filter((s) => s.id !== serviceId), p.status),
            }
          : p
      )
    )
  ...
  const addPaymentToProtocol = (
    ...
    const id = ++serviceIdSeq
    const newPayment: ProtocolPayment = { ...payment, paymentType: normalizePaymentType(payment.paymentType), id, protocolId }
    setProtocols((prev) =>
      prev.map((p) =>
