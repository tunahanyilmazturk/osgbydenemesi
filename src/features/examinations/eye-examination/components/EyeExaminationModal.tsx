...
import type { ProtocolService, EyeExaminationData } from '@/shared/types'
import {
...
export function EyeExaminationModal(props: EyeExaminationModalProps) {
  if (!props.isOpen || !props.service) return null
  return <EyeExaminationModalContent {...props} service={props.service} />
}

function EyeExaminationModalContent({ isOpen, onClose, service, patientName, patientTc, patientBirthDate, patientGender, company, protocolNo, examType, onSave }: EyeExaminationModalProps & { service: ProtocolService }) {
  const [data, setData] = useState<EyeExaminationData>(() => parseEyeExaminationData(service))
  ...
  const addTemplate = (category: TemplateCategory) => {
    ...
    if (!text) return
    const newTemplates = [...templates, { id: crypto.randomUUID(), category, text }]
  ...
  const addExtraFinding = (label: string, values: string[]) => {
    const id = `ef_${crypto.randomUUID()}`
    setData((prev) => ({
      ...prev,
