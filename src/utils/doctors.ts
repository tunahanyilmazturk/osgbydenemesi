...
// Sabit test türleri — geri uyumluluk için (esnek string tipine geçildi)
export const TEST_TYPES = [
  'GÖZ TARAMASI (otorefraktometre)',
  'İşitme Testi (ODYOMETRİ)',
] as const
...
// Hizmet kataloğundan tüm hizmet adlarını getir (dinamik test türleri)
export function loadServiceNames(): string[] {
  try {
    const raw = localStorage.getItem('cetka-service-catalog')
    if (raw) {
      const parsed = JSON.parse(raw)
...
// Tüm seçilebilir test türlerini getir — hizmet kataloğu + sabit türler (tekrarsız)
export function getAvailableTestTypes(): string[] {
  const serviceNames = loadServiceNames()
  const all = [...serviceNames, ...TEST_TYPES]
...
export function getDoctorForTest(testType: string): Doctor | undefined {
  ...
  const main = doctors.find((d) => d.testType === testType)
  if (main) return main
  // Yoksa asistanlardan ara — bağlı doktoru döndür
  for (const d of doctors) {
    const ast = d.assistants?.find((a) => a.testType === testType)
    if (ast) return d
  }
  return undefined
}
