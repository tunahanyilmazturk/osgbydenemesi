import type { EyeExaminationData, PatientDetail, Protocol, ProtocolService } from '../types'
...
export async function openEyeExaminationPdf(report: ReportData, existingDoc?: import('jspdf').jsPDF, isFirstPage = false, returnBlob = false): Promise<import('jspdf').jsPDF | Blob | void> {
  ...
  // Stamp areas: left = Doctor, right = Assistant/Operator
  const stampW = 40
  const stampH = 14
  ...
  const doctorStamp = getDoctorStampForTest('GÖZ TARAMASI (otorefraktometre)')
  // Sağ kaşe: Test'e atanmış asistanın kaşesi (kullanıcıdan)
  const assistantStamp = getAssistantStampForTest('GÖZ TARAMASI (otorefraktometre)')
