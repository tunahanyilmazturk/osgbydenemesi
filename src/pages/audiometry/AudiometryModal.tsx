...
import type { ProtocolService, AudiometryData } from '../../types'
...
export function AudiometryModal({
  isOpen,
  onClose,
  service,
  patientName,
  ...
  examType,
  onSave,
}: AudiometryModalProps) {
  const [data, setData] = useState<AudiometryData>({ ...defaultData })
  const { canApproveAudiometry } = useAuth()

  useEffect(() => {
    if (isOpen && service) {
      setData(parseAudiometryData(service))
    }
  }, [isOpen, service])
