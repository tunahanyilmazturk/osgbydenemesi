...
import { useServices, defaultTubeTypes } from '@/state/ServicesContext'
import { useToast } from '@/state/ToastContext'
import { useConfirm } from '@/state/ConfirmContext'
import { Input } from '@/shared/components/ui/Input'
import { Modal } from '@/shared/components/ui/Modal'
import { PageHeader } from '@/shared/components/PageHeader'
import type { ServiceTubeType } from '@/shared/types'
...
export function ServiceTubeTypes() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (tubeTypes.some((tube) => tube.id !== editingId && tube.name.trim().toLocaleLowerCase('tr-TR') === normalizedName)) {
      showToast('warning', 'Tüp tipi zaten mevcut', 'Aynı adla ikinci bir tüp tipi oluşturamazsınız.')
      return
    }
    const barcodeShortName = form.barcodeShortName?.trim().toLocaleUpperCase('tr-TR') ?? ''
    if (barcodeShortName && tubeTypes.some((tube) => tube.id !== editingId && tube.barcodeShortName?.trim().toLocaleUpperCase('tr-TR') === barcodeShortName)) {
      showToast('warning', 'Barkod kısa adı kullanılıyor', 'Her tüp tipi için farklı bir barkod kısa adı girin.')
      return
    }
    const normalizedForm = { ...form, name: form.name.trim(), barcodeShortName }
    if (editingId) {
      updateTubeType(editingId, normalizedForm)
      showToast('success', 'Tüp tipi güncellendi', `"${form.name}" kaydedildi.`)
    } else {
      addTubeType(normalizedForm)
